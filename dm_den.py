...
def build_direcs(suffix, res, mass_class, typ='fire'):
    ...
    if mass_class>10:
        ...
        direc='/data17/grenache/aalazar/FIRE/GV'+typ_char+'/m'+mass_class+suffix+'_res'+res+\
              '/output/hdf5/'
    elif mass_class==10:
        if typ=='dmo':
...
def unpack_gas(df, galname, dr=1.5, drsolar=None, typ='fire'):
    getparts=['PartType0']
    suffix=df.loc[galname,'fsuffix']
    res=df.loc[galname,'res']
    host_key=df.loc[galname,'host_key']
...
def calc_temps(he_fracs, e_abundances, energies):
    ...
    Ts = (mean_molecular_weights.si * (gamma-1.) * energies / c.k_B.si).value
    return Ts
